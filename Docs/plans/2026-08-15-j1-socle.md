...
> **Jalon terminé et fusionné dans `main`. Document d'archive : il décrit ce qui
> a été fait, pas l'état actuel du dépôt.** Deux parties ont depuis été
> remplacées et ne doivent pas servir de référence :
> la couche visuelle (les tokens `:root`, `genererCssTokens()`, `base.css` et
> les coquilles d'écran) — voir *Kolek Design System v2* §8.2 pour la
> correspondance ; et la commande `verifier:j1`, devenue `npm run verifier`.
> Le schéma, les migrations et les tests de base restent, eux, décrits ici
> fidèlement.
...
-- Cible de la clé étrangère composite de cartes. Redondant avec la clé primaire,
-- mais PostgreSQL exige une contrainte d'unicité sur le couple exact référencé.
...
create table public.cartes (
  ...
  collecteur_id    uuid not null references public.collecteurs(id) on delete cascade,
  client_id        uuid not null,          -- lié au collecteur par la contrainte composite plus bas
  ...
  -- Le client d'une carte doit appartenir au même collecteur que la carte. Sans
  -- ce lien, un collecteur pourrait ouvrir une carte sur le client d'un autre :
  -- RLS la masquerait, mais l'index unique « une carte active par client » étant
  -- global, le collecteur légitime serait définitivement bloqué sur son propre
  -- client, avec une erreur portant sur une ligne invisible pour lui.
