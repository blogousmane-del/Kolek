  ...
  return (
    ...
    >
      ...
      <p className="mb-12 max-w-xl font-body text-base text-muted-foreground">
        Kolek se paie comme un outil : un prix fixe par mois, en FCFA. Jamais un
        pourcentage sur ce que tes clients te confient.
      </p>
          ...
          return (
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <h3 className="font-headings text-xl font-bold">
                  {palier.nom}
                </h3>
                {vedette && (
              ...
              <p
                className={`mb-1 font-headings text-4xl font-bold tabular-nums ${vedette ? "text-or" : "text-ink"}`}
              >
                {palier.prix === 0 ? "Gratuit" : formatMontant(palier.prix)}
                {palier.prix > 0 && (
                  <span
                    className={`ml-1 font-body text-sm font-medium ${vedette ? "text-white/50" : "text-muted-foreground"}`}
                  >
                    FCFA/{palier.periode}
                  </span>
                )}
              ...
              <p
                className={`mb-6 font-body text-sm ${vedette ? "text-white/60" : "text-muted-foreground"}`}
              >
                {palier.accroche}
              </p>
      ...
      <p className="mt-8 font-body text-sm text-muted-foreground">
        Les comptes sont ouverts par l’équipe GTCS après un premier échange : le
        premier mois est un essai, sans engagement.{" "}
        <a
          href={APP_COLLECTEUR}
