...
describe("le formulaire d’ouverture", () => {
  ...
  it("garde le nom et le numéro obligatoires", () => {
    ...
    expect(
      (screen.getByLabelText(/nom complet/i) as HTMLInputElement).required,
    ).toBe(true);
    expect(
      (screen.getByLabelText(/ton numéro/i) as HTMLInputElement).required,
    ).toBe(true);
  });
  ...
  it("laisse la zone et le message facultatifs", () => {
    ...
    expect(
      (screen.getByLabelText(/zone de collecte/i) as HTMLInputElement).required,
    ).toBe(false);
    expect(
      (screen.getByLabelText(/un mot sur ton activité/i) as HTMLTextAreaElement)
        .required,
    ).toBe(false);
  });
