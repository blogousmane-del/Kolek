...
/**
 * L'écran du collecteur, dans un téléphone, dès le premier regard.
 *
 * ## Pourquoi il existe
 *
 * L'audit du 2026-09-02 a compté zéro image sur tout le site : ni marché, ni
 * téléphone, ni écran. Une page qui vend un outil de terrain sans jamais le
 * montrer demande au visiteur de croire sur parole.
 *
 * ## Pourquoi ce n'est pas une capture d'écran
 *
 * Une capture se périme au premier changement d'interface et personne ne s'en
 * aperçoit : la page continue de montrer un produit qui n'existe plus. Ici
 * l'écran est rendu par `CarteCollecte`, **le composant que le collecteur voit
 * vraiment**. Le jour où la carte de collecte change, cette vitrine change avec
 * elle, sans que quiconque ait à y penser.
 *
 * ## Pourquoi les chiffres sont calculés
 *
 * `soldeRestituable` est la fonction du moteur, pas un nombre recopié. Une carte
 * de 2 000 FCFA à sa douzième mise rend 22 000 FCFA parce que la première mise
 * est la commission du collecteur — c'est la règle du produit, et la page ne
 * sait pas l'écrire autrement. Principe 7 du système de design : ne jamais
 * afficher un chiffre qu'on ne sait pas.
 *
 * Le nom, lui, est inventé. Aucune donnée réelle ne sort vers ce site ; sa CSP
 * l'interdit.
 */
      ...
      <div className="relative mx-auto w-[320px] max-w-full rounded-[2.75rem] border border-white/15 bg-dark-canvas p-2.5 shadow-[0_30px_80px_-20px_rgba(210,178,76,0.25)]">
        <div className="overflow-hidden rounded-[2.25rem] bg-canvas" aria-hidden>
          ...
          <div className="px-4 pb-5 pt-4">
            ...
            {/* Pas de champ de saisie : le montant est imposé par la carte, et la
                carte l'affiche déjà. Le répéter allongeait l'appareil au point de
                pousser le titre hors du premier écran sur un portable. */}
