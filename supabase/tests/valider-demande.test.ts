...
describe('ce qui passe', () => {
  ...
  it('accepte le strict nécessaire : un nom et un numéro', () => {
    const r = validerDemande({ nom: 'Adama', telephone: '0701020304' });
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    // Ce qu'il faut pour rappeler quelqu'un à Abidjan, et rien de plus. Zone et
    // message vides deviennent `null`, pas des chaînes vides : la base
  ...
  it('coupe les espaces de bord', () => {
    const r = validerDemande({ nom: '  Fatou  ', telephone: ' 0701020304 ' });
    expect(r.ok).toBe(true);
    if (!r.ok) return;
